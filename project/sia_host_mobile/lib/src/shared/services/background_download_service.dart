...
//   static const _activeDownloadsKey = 'active_downloads';

//   static Future<Set<String>> _getActiveDownloads() async {
//     final prefs = await SharedPreferences.getInstance();
//     final activeDownloads = prefs.getStringList(_activeDownloadsKey) ?? [];
//     return activeDownloads.toSet();
//   }

//   static Future<void> _setActiveDownloads(Set<String> activeDownloads) async {
//     final prefs = await SharedPreferences.getInstance();
...
//   static Future<int> _nextNotificationId() async {
//     final prefs = await SharedPreferences.getInstance();

//     const keyToUse = 'last_notification_id';

//     final last = prefs.getInt(keyToUse) ?? 0;
//     final next = last + 1;

//     await prefs.setInt(keyToUse, next);
//     return next;
//   }

//   static Future<int> _currentNotificationId() async {
//     final prefs = await SharedPreferences.getInstance();

//     const keyToUse = 'last_notification_id';

//     final current = prefs.getInt(keyToUse) ?? 0;
//     return current;
//   }

//   /// Enqueue a download task.
//   ///
//   /// This method will check if the download is already in progress.
//   /// If not, it will register a one-off task with WorkManager.
...
//   }) async {
//     final activeDownloads = await _getActiveDownloads();
//     Logger().i('Active downloads: $activeDownloads');
//     Logger().i('Current notification ID: ${await _currentNotificationId()}');

//     // remove all space in the file name to make a slug-like fileId
//     final fileId = fileObject.name.replaceAll(' ', '_');
//     // if (activeDownloads.contains(fileId)) return false;
...
//       );
//       await _setActiveDownloads(activeDownloads);
//       return true;
//     } catch (e) {
...
//   /// Handle the background download task.
//   ///
...
//     final notifId = await _nextNotificationId();
//     final activeDownloads = await _getActiveDownloads();

//     Logger().w('Active downloads: $activeDownloads');
//     Logger().w('current ID: $notifId');
//     try {
//       // Show start _notification
...
//         body: 'Failed to download $saveAs',
//       );
//     } finally {
//       final removed = activeDownloads.remove(fileId);
//       log('Removed $removed | ID: $fileId');
//       await _setActiveDownloads(activeDownloads);

//       Logger().w('Active downloads: ${await _getActiveDownloads()}');
//     }
//   }
